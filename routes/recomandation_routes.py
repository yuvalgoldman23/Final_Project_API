# routes/recomandation_routes.py
...
recomandation_routes = Blueprint('recomandation_routes', __name__)
...
Recomandation_canidentes=cursor2.fetchall()
api_key = '2e07ce71cc9f7b5a418b824c87bcb76f'


for r in  Recomandation_canidentes:
    r['weight'] = math.sqrt(r['num_raters'])
total_weight = sum(r['weight'] for r in Recomandation_canidentes)
for r in  Recomandation_canidentes:
...
@recomandation_routes.route('/api/recomandation_feedback', methods=['GET'])
...
@recomandation_routes.route('/api/Media_recomandation', methods=['GET'])
def get_media_recomandationv2():
    ...
    while 1:
        if len(algo_recomandation)== 5:
            break
        can= greedy_random_selection(Recomandation_canidentes)
        can_gnr_str=""
        if (can["is_movie"]) :
